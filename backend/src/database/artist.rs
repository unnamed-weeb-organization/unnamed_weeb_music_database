use crate::{
    models::{artist::{Artist, ArtistIden, SongArtistIden, Options}, release::SongReleaseIden, song::SongIden},
    utils::error::Error,
};
...
pub async fn get_artists_by_song_id(id: &Ulid, db: &PgPool) -> Result<Vec<Artist>, Error> {
	let (query, values) = Query::select()
		.columns([(ArtistIden::Table, ArtistIden::Id), (ArtistIden::Table, ArtistIden::Name)])
		.from(SongArtistIden::Table)
		.join(JoinType::LeftJoin, ArtistIden::Table, Expr::col(SongArtistIden::ArtistId).equals(ArtistIden::Table, ArtistIden::Id))
...
fn build_query(options: &Options) -> (String, Values) {
	...
	if let Some(song_id) = &options.song_id {
		q.join(JoinType::LeftJoin,SongIden::Table, Expr::tbl(SongArtistIden::Table, SongArtistIden::ArtistId).equals(ArtistIden::Table, ArtistIden::Id))
			.and_where(Expr::col(SongArtistIden::SongId).eq(song_id.clone()));
	}

	if let Some(release_id) = &options.release_id {
		q.join(JoinType::LeftJoin,ArtistIden::Table, Expr::tbl(SongReleaseIden::Table, SongReleaseIden::SongId).equals(ArtistIden::Table, ArtistIden::Id))
