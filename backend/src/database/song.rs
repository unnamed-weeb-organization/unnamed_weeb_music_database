use crate::{
    models::{song::{Song, SongIden, Options}, Name, artist::SongArtistIden, release::SongReleaseIden},
    utils::error::Error,
};
...
pub async fn create_song(ulid: ulid::Ulid, name: Name, _artists: Vec<String>, _releases: Option<Vec<String>>, db: &PgPool) -> Result<Song, Error> {
	let (query, values) = Query::insert()
		...
		.values_panic(vec![
			ulid.to_string().into(),
			vec![name.native.clone(), name.romanized.clone(), name.english.clone()].into(),
		])
...
fn build_query(options: &Options) -> (String, Values) {
	...
	if let Some(artist_id) = &options.artist_id {
		q.join(JoinType::LeftJoin,SongIden::Table, Expr::tbl(SongArtistIden::Table, SongArtistIden::SongId).equals(SongIden::Table, SongIden::Id))
			.and_where(Expr::col(SongArtistIden::ArtistId).eq(artist_id.clone()));
	}

	if let Some(release_id) = &options.release_id {
		q.join(JoinType::LeftJoin,SongIden::Table, Expr::tbl(SongReleaseIden::Table, SongReleaseIden::SongId).equals(SongIden::Table, SongIden::Id))
